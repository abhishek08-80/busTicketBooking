...
export default class customerController {
    ...
    try {
      ...
      if (data == 'userDoesNotExist') {
        res
          .status(statusCode.notFound)
          .json(
            failResponse(statusCode.badRequest, data, message.notExist('customer')),
          );
      } else {
        res
          .status(statusCode.success)
          .json(successResponse(statusCode.success, data, message.fetch('customer')));
      }
    } catch (err) {
      logger.error(message.errorLog('userAdd', 'userController', err));
      res
  ...
  public static async loginCustomer(req: Request, res: Response) {
    try {
